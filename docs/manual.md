...
-   **`LUA_GCGEN` (int minormul, int majormul):** Changes the collector
    ...
    [§2.5.2](#2.5.2)). Returns the previous mode (`LUA_GCGEN` or
    `LUA_GCINC`).
...
error messages and in debug information (see [§4.7](#4.7)).
...
the registry (see [§4.3](#4.3)). When loading main chunks, this upvalue
...
alive (see [§2.5](#2.5)). Moreover, if the userdata is marked for
finalization (see [§2.5.3](#2.5.3)), its address is valid at least until
...
`n` tells how many upvalues this function will have (see [§4.2](#4.2)).
...
string (see [§4.1.3](#4.1.3)). It is similar to the ISO C function
...
[§3.3.8](#3.3.8)). Like a to-be-closed variable in Lua, the value at
...
[§4.1.3](#4.1.3)). This string always has a zero (\'`\0`\') after its
...
running function (see [§4.2](#4.2)). `i` must be in the range
*\[1,256\]*.
...
code in case of errors (see [§4.4.1](#4.4.1)).
...
is equivalent to the \'`#`\' operator in Lua (see [§3.4.7](#3.4.7)).
...
by the function (see [§4.1.3](#4.1.3)). If `len` is not `NULL`, the
...
-   **\"`incremental`\":** Change the collector mode to incremental.
    ...
    [§2.5.1](#2.5.1)). A zero means to not change that value.
-   **\"`generational`\":** Change the collector mode to generational.
    This option can be followed by two numbers: the garbage-collector
    minor multiplier and the major multiplier (see [§2.5.2](#2.5.2)). A
    zero means to not change that value.

See [§2.5](#2.5) for more details about garbage collection and some of
these options.
...
Raises an error (see [§2.3](#2.3)) with `message` as the error object.
This function never returns.
...
debug information (see [§4.7](#4.7)). When absent, it defaults to
...
### [`coroutine.close (co)`]{#pdf-coroutine.close}
...
### [`coroutine.isyieldable ([co])`]{#pdf-coroutine.isyieldable}
...
### [`string.gmatch (s, pattern [, init])`]{#pdf-string.gmatch}
...
Looks for the first *match* of the `pattern` (see [§6.4.1](#6.4.1)) in
...
### [`utf8.codes (s [, lax])`]{#pdf-utf8.codes}
...
### [`utf8.codepoint (s [, i [, j [, lax]]])`]{#pdf-utf8.codepoint}
...
### [`utf8.len (s [, i [, j [, lax]]])`]{#pdf-utf8.len}
...
### [`math.randomseed ([x [, y]])`]{#pdf-math.randomseed}
...
([§4.7](#4.7)) to Lua programs. You should exert care when using this
...
### [`debug.getuservalue (u, n)`]{#pdf-debug.getuservalue}
...
### [`debug.setuservalue (udata, value, n)`]{#pdf-debug.setuservalue}
